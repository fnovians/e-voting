  ...
  <div class="row g-4 mt-3 border-top border-glass pt-4">
    <div class="col-md-4">
      <div class="d-flex align-items-center gap-3">
        ...
        <div>
          <span class="text-muted small-text d-block text-uppercase fw-bold tracking-wider">Total Kandidat</span>
          <strong class="text-dark fs-5">{{ count($candidates) }} Pasangan Calon</strong>
        </div>
      </div>
    </div>
    <div class="col-md-4">
...
<div class="voter-status-bar d-flex justify-content-between align-items-center mb-5 p-4 rounded-4 shadow-sm border-glass">
  ...
  <span class="badge rounded-pill bg-warning-subtle text-warning fw-bold border border-warning-subtle px-3 py-2" style="background-color: #fef3c7; color: #d97706; font-size: 0.78rem;">Belum Memilih</span>
</div>

<!-- Candidate Section (Horizontal Premium Stripe Layout) -->
<h3 class="fw-bold text-dark mb-4" style="letter-spacing: -0.5px;">Kandidat Calon Ketua HIMA</h3>
<div class="row g-4 mb-5">
  @forelse ($candidates as $cand)
    <div class="col-12">
      <div class="card p-4 candidate-card border-glass shadow-sm">
        <!-- Left: Candidate Photo or Avatar -->
        @if ($cand->photo)
          <img src="{{ asset($cand->photo) }}" alt="{{ $cand->name }}" class="candidate-avatar" style="object-fit: cover;">
        @else
          <div class="candidate-avatar">{{ $cand->id }}</div>
        @endif
        
        <!-- Middle: Candidate details -->
        <div class="candidate-body">
          <h4 class="text-dark fw-bold mb-1" style="letter-spacing: -0.5px;">{{ $cand->name }}</h4>
          <span class="text-muted d-block small mb-2 italic">"Mewujudkan visi digitalisasi & inovasi Informatika"</span>
        ...
        <div class="d-flex flex-column sm-flex-row gap-2 flex-shrink-0 align-self-stretch justify-content-center border-start border-glass ps-4">
          ...
          <button class="btn btn-cyan btn-sm py-2 px-3 fw-bold" onclick="openConfirmationModal('{{ $cand->id }}', '{{ $cand->name }}')" {{ !$votingOpen ? 'disabled' : '' }}>
            Pilih Kandidat
          </button>
        </div>
      </div>
        ...
        <div class="modal-content shadow-box border-glass">
          <div class="modal-header border-glass px-4 py-3">
            <h5 class="modal-title fw-bold text-dark d-flex align-items-center gap-2">
              <span class="badge bg-primary-subtle text-primary border border-primary-subtle rounded-circle d-inline-flex align-items-center justify-content-center" style="width: 28px; height: 28px; background: rgba(37,99,235,0.08);">{{ $cand->id }}</span>
          ...
          <div class="modal-body p-4 text-start">
            ...
            @endif
            <h4 class="text-dark fw-bold mb-3 {{ $cand->photo ? 'text-center' : '' }}">{{ $cand->name }}</h4>
            <div class="mb-4">
          ...
          <div class="modal-footer border-glass px-4 py-3">
            <button type="button" class="btn btn-secondary btn-sm py-2 px-3 border border-glass bg-light text-dark fw-bold" data-bs-dismiss="modal">Tutup</button>
            <button type="button" class="btn btn-cyan btn-sm py-2 px-4 fw-bold" data-bs-dismiss="modal" onclick="openConfirmationModal('{{ $cand->id }}', '{{ $cand->name }}')" {{ !$votingOpen ? 'disabled' : '' }}>Pilih Paslon</button>
          </div>
        </div>
      </div>
    </div>
  @empty
    <div class="col-12 text-center text-muted card p-4 border-glass">Belum ada kandidat terdaftar.</div>
  @endforelse
</div>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content shadow-box border-glass">
      ...
      <div class="modal-body p-4 text-center">
        ...
        <h4 class="text-dark fw-bold mb-3" id="confirm-candidate-name">Nama Kandidat</h4>
...
<script>
  function openConfirmationModal(id, name) {
